    ...
    <section class="max-w-7xl w-full my-16 mx-auto px-4 sm:px- mb-20 lg:px-4">
      ...
      <div>
        <div :id="'accordion-collapse'" data-accordion="open">
          <div v-for="(category, index) in allDocumentCategories?.filter((document: any) => document.id === parseInt(documentId))" :key="category.id" class="mb-6">
              ...
              <div v-if="openAccordion === index" :id="`accordion-collapse-body-${index + 1}`" class=""
                ...
                <div
                  ...
                  <div v-for="document in category?.documents" class="flex flex-col gap-2">
                    <a :href="`${imagesUrl}/uploads/${document?.filename}`" target="_blank"
...
const documentId: any = computed(() => decryptString(route.params.id.toString()));
...
axios.get(`${url}/document-category`)
  .then((response: any) => {
    ...
    console.log(allDocumentCategories.value);
  })
  .catch((error: string) => {
    console.error(error);
  });
