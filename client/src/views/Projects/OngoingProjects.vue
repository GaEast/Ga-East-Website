    ...
    <div class="bg-white border-b border-gray-100 py-10">
      <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-center gap-2 text-xs text-gray-400 mb-4">
          ...
          <span class="text-[#6CC551]">Ongoing Projects</span>
        </div>
        <p class="text-[#6CC551] text-xs font-bold uppercase tracking-widest mb-2 text-center">In Progress</p>
        ...
        <p class="mt-3 text-gray-500 text-sm max-w-2xl leading-relaxed text-center mx-auto">
          Projects currently being executed across the Ga East Municipality.
        </p>
      </div>
    </div>
    ...
    <section class="py-12 bg-gray-50 min-h-screen">
      <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div v-else-if="projects.length === 0"
          ...
          <p class="text-gray-500 font-medium text-sm">No ongoing projects at the moment.</p>
...
const fetchProjects = async () => {
  ...
  try {
    const res = await axios.get(`${url}/posts`, {
      params: { category: 'ONGOING PROJECT', page: currentPage.value, limit: perPage.value }
    });
